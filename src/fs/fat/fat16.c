...
#define FAT_FILE_READ_ONLY 0x1
#define FAT_FILE_HIDDEN 0x02
#define FAT_FILE_SYSTEM 0x04
#define FAT_FILE_VOLUME_LABEL 0x8
#define FAT_FILE_SUBDIRECTORY 0x10
#define FAT_FILE_ARCHIEVED  0x20
...
struct fat_header
{
    ...
    uint8_t oem_identified[8];
    uint16_t bytes_per_sector;
    uint8_t sectors_per_cluster;
    uint16_t reserved_sectors;
    uint8_t fat_copies;
    uint16_t rood_dir_entries; // number of fat directory entries in root director? 
                                // will it be changed every time a new directory is added in root direc?
...
struct fat_directory
{
    ...
    int total; // what is this total? number of entries in the directory?
    int sector_pos;
...
static void fat16_init_private(struct disk* disk, struct fat_private* private)
{
    ...
    private->cluster_read_stream = diskSteamerNew(disk->id);
    private->fat_read_stream = diskSteamerNew(disk->id);
    private->directory_stream = diskSteamerNew(disk->id);
...
int fat16_get_total_items_for_directory(struct disk* disk, int directory_start_sector)
{
    struct fat_directory_item item;
    ...
    int total_valid_entries = 0;
    int res = 0;
    struct fat_private* fat_private = disk->fs_private;
    struct disk_stream* stream = fat_private->directory_stream;
    if (diskSteamerSeek(stream, fat16_sector_to_absolute(disk, directory_start_sector)) != PEACHOS_ALL_OK)
    {
       res = -EIO;
       goto out;
    }
    ...
    while(1)
    {
        if (diskSteamerRead(stream, &item, sizeof(item)) != PEACHOS_ALL_OK)
        {
            res = -EIO;
            goto out;
        }
...
int fat16_get_root_directory(struct disk* disk, struct fat_private* private, struct fat_directory* directory)
{
    ...
    int root_dir_entries = primary_header->rood_dir_entries;
    ...
    int total_entries = fat16_get_total_items_for_directory(disk, root_dir_sector_pos);
    ...
    if (diskSteamerSeek(stream, fat16_sector_to_absolute(disk, root_dir_sector_pos)) != PEACHOS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }

    if (diskSteamerRead(stream, dir, root_dir_size) != PEACHOS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
    ...
    directory->item = dir;
    directory->total = total_entries;
    directory->sector_pos = root_dir_sector_pos;
    // why it is root_dir_sector_pos + total_sectors ? cause it's the first sector after dirctory sector end
...
int fat16_resolve(struct disk* disk)
{
   ...
   struct disk_stream* stream = diskSteamerNew(disk->id);
   if (!stream)
   {
        res = -ENOMEM;
        goto out;
   ...
   if (diskSteamerRead(stream, &(fat_private->header), sizeof(fat_private->header)) != PEACHOS_ALL_OK)
   {
        res = -EIO;
        goto out;
   }
...
void fat16_get_full_filename_with_extension(struct fat_directory_item* item, char* out, int max_len)
{
    memset(out, 0x0, max_len);
    char *out_temp = out;
...
{
    // read the manual why -2 from cluster here
...
{
    // here it should be private->header.primary_header.reserved_sectors + 1 (for boot sector) right?
    return private->header.primary_header.reserved_sectors + 1;
}
static int fat16_get_fat_entry(struct disk* disk, int cluster)
{
    ...
    uint32_t fat_table_position = fat16_get_first_fat_sector(private) * disk->sectorSize;
    // shouldn't it be fat_table_position + (cluster * PEACHOS_FAT16_FAT_ENTRY_SIZE)?
    res = diskSteamerSeek(stream, fat_table_position * (cluster * PEACHOS_FAT16_FAT_ENTRY_SIZE));
    if (res < 0)
    {
        goto out;
    }
    ...
    uint16_t result = 0;
    res = diskSteamerRead(stream, &result, sizeof(result));
    if (res < 0)
    {
        goto out;
    }
...
static int fat16_get_cluster_for_offset(struct disk* disk, int starting_cluster, int offset)
{
    int res = 0;
    struct fat_private* private = disk->fs_private;
    int size_of_cluster_bytes = private->header.primary_header.sectors_per_cluster * disk->sectorSize;
    ...
    for (int i = 0 ; i < clusters_ahead; i++)
    {
        ...
        {
            // why don't we use this cluster?
            // we are at the last entry
            res = - EIO;
            goto out;
        }

        // cluster is marked as bad
...
static int fat16_read_internal_from_stream(struct disk* disk, struct disk_stream* stream, int cluster, int offset, int total, void* out)
{
    ...
    res = diskSteamerSeek(stream, starting_pos);
    if (res != PEACHOS_ALL_OK)
    {
        goto out;
    }

    res = diskSteamerRead(stream, out, total_to_read);
    if (res != PEACHOS_ALL_OK)
    {
        goto out;
    }
...
// reads as many clusters as requied i.e total paramter bytes, with offset from first cluster
...
struct fat_directory* fat16_load_fat_directory(struct disk* disk, struct fat_directory_item* item)
{
    ...
    int directory_size_bytes = directory->total * sizeof(struct fat_directory_item);
    directory->item = kZalloc(directory_size_bytes);
    if (!directory->item)
    {
        goto out;
    }

    fat16_read_internal(disk, cluster, 0x00, directory_size_bytes, directory->item);
...
struct fat_item* fat16_new_fat_item_for_directory_item(struct disk* disk, struct fat_directory_item* item)
{
    ...
    if (item->attribute & FAT_FILE_SUBDIRECTORY)
    {
        // This is a directory, load the directory
...
struct fat_item* fat16_find_item_in_directory(struct disk* disk, struct fat_directory* fat_directory, const char* name)
{
    struct fat_item* f_item = 0;
    char temp_filename[PEACHOS_MAX_PATH_LENGTH];
    for (uint32_t i = 0; i < fat_directory->total; i++)
    {
        fat16_get_full_filename_with_extension(&fat_directory->item[i], temp_filename, sizeof(temp_filename));
        if (istrncmp(temp_filename, name, sizeof(temp_filename)) == 0)
        {
            // found it, let's create a new fat item
            f_item = fat16_new_fat_item_for_directory_item(disk, &fat_directory->item[i]);
        }
    }
...
struct fat_item* fat16_get_directory_entry(struct disk* disk, struct path_part* path)
{
    ...
    // code for others parts, i.e other folders until we arrive at file
